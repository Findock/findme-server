...
@Injectable()
export class FindMeDistinctiveFeaturesService {
    public constructor(
        ...
        this.distinctiveFeaturesRepository.count().then(async count => {
            ...
            distinctiveFeaturesDataset.forEach(async distinctiveFeature => {
                await this.createDistinctiveFeature(distinctiveFeature.namePl);
            });
        });
    }

    public async createDistinctiveFeature(namePl: string): Promise<FindMeDistinctiveFeature> {
        const distinctiveFeature = this.distinctiveFeaturesRepository.create({ namePl });
